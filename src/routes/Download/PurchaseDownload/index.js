...
const PurchaseDownload = (props) => {
  let history = useHistory();
  var today = new Date(),

  date = today.getFullYear() + '-' + (today.getMonth() + 1) + '-' + today.getDate();
  const [receiptsdwn, setPurchaseDownload] = useState({
   purchase_from_date: "2023-04-01",
   purchase_to_date: date,
   purchase_sub_item: "",
   purchase_sub_unit: "",
...
if(v){
  setIsButtonDisabled(true)
    axios({
     ...
     link.click();
      NotificationManager.success("Purchase is Downloaded Successfully");
        setIsButtonDisabled(false)
    
    }).catch((err) =>{
     NotificationManager.error("Purchase is Not Downloaded");
     setIsButtonDisabled(false)
   });
  }
  };
...
if(v){
  setIsButtonDisabled(true)
    axios({
     ...
     document.body.appendChild(link);
     link.click();
      NotificationManager.success("Purchase Detail is Downloaded Successfully");
        setIsButtonDisabled(false)
    
    }).catch((err) =>{
     NotificationManager.error("Purchase Detail is Not Downloaded");
     setIsButtonDisabled(false)
   });
  }
  };
  ...
  return (
    <div className="textfields-wrapper">
      <PageTitleBar title="Download Purchase" match={props.match} />
      <RctCollapsibleCard>
