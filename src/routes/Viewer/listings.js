...
export default class NewListViewer extends React.Component {
				  ...
				  customBodyRender: (value) => {
					return (	
					  ...
					  <Helmet>
                        ...
                        <meta name="description" content="FTS Viewers" />
		              </Helmet>
   ...
   showAddViewerModal(){
	...
	getData=()=>{
		let result=[]
		axios({
			url:baseURL+"/superadmin-get-all-viewers",
			method: "GET",
			headers: {
			...
			for(let i = 0; i < response.length; i++){
				tempRows.push([
					...
					response[i]["user_position"],
					response[i]["email"],
					response[i]["phone"],
					response[i]["viewer_chapter_ids"],
					Moment(response[i]["viewer_start_date"]).format('DD-MM-YYYY'),
					Moment(response[i]["viewer_end_date"]).format('DD-MM-YYYY') ,
					response[i]["id"],
				]);
			}
	...
	render() {
		...
		return (
			...
			{loader && 
              ...
              <Link className="btn btn-outline-light" to="addviewer">
			  <Button variant="contained"  className="text-white bg-success px-3 btn-xs" style={{marginBottom:30}}>
                   Add Viewer</Button>
              </Link>
            ...
            >
               ...
               <h2>Adding a New Viewer</h2>
                  ...
                  <Button variant="contained" type="submit" onClick={this.addTheViewer} className="text-white bg-success px-3 btn-xs" style={{marginLeft:10, marginBottom:10}}>
                     Submit
                  </Button>
